...
func TestExternalFrontendHandlerServesSPA(t *testing.T) {
	dir := t.TempDir()
	mustWriteFrontend(t, filepath.Join(dir, "index.html"), "<html>gmux</html>")
	mustWriteFrontend(t, filepath.Join(dir, "manifest.json"), `{"name":"gmux"}`)
	mustWriteFrontend(t, filepath.Join(dir, "sw.js"), `self.addEventListener('fetch', () => {})`)
	if err := os.Mkdir(filepath.Join(dir, "assets"), 0o755); err != nil {
		t.Fatal(err)
	}
	mustWriteFrontend(t, filepath.Join(dir, "assets", "app.js"), "console.log('gmux')")
...
func mustWriteFrontend(t *testing.T, path, data string) {
	t.Helper()
