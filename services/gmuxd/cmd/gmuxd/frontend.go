...
func frontendCacheControl(name string) string {
	...
	case "index.html", "manifest.json", "sw.js":
		// Installed/PWA windows do not have browser chrome refresh controls, so
		// these update-entry files must revalidate on every open/reload. Hashed
		// bundle assets below can remain immutable.
		return "no-cache"
	}
