...
func cmdWorktreeCreate(c *command) int {
	...
	if result.SessionID != "" {
		fmt.Printf("session %s\n", displayID(cliSession{ID: result.SessionID}))
	}
	return 0
}
...
func sendWorktreePrompt(id, _ string, prompt string) error {
	body, err := json.Marshal(map[string]any{"prompt": prompt, "mode": "prompt", "wait": false})
	if err != nil {
		return err
	}
	client := gmuxdClient()
	client.Timeout = 0
	resp, err := client.Post(gmuxdBaseURL()+"/v1/sessions/"+id+"/prompt", "application/json", bytes.NewReader(body))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
